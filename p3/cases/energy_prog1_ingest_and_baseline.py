...
Overview (matches this chat):
  - Reads './cases/bus/energy/house_usage_sample.json' (or your file via --in).
  - Normalizes per-appliance usage to kWh/day.
  - Computes daily cost and daily CO₂.
  - Prints Answer/Reasons/Check; writes './cases/bus/energy/baseline_daily.json' for Program 2.
...
def main():
    ...
    ap.add_argument("--in", dest="infile", default="./cases/bus/energy/house_usage_sample.json")
    ap.add_argument("--out", default="./cases/bus/energy/baseline_daily.json")
    args = ap.parse_args()

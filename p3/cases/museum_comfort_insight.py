...
# -*- coding: utf-8 -*-
"""
Museum Comfort Insight — Compact, EYE-compatible demonstrator
=============================================================

STORY
-----
A visitor enters **MuseoCity** and taps on the **"Impressionists"** gallery in
the app. The phone already knows two harmless bits of public context for this
tile: the gallery's **crowd level** (0–10) and the **wait** to enter. It does a
tiny, local calculation — no cloud, no personal profile — a quick **comfort
index**:

    comfortIndex = (crowdLevel × 10) + waitMinutes

If the selected gallery's comfort index exceeds a policy threshold, the app
shows a gentle banner and suggests a **calmer nearby gallery**. What leaves the
device isn’t any personal preference or movement trace — just a signed,
audience-scoped **insight envelope**: who it's for (MuseoCity), what it allows
(show a comfort banner and suggestion), when it expires, and a compact set of
assertions (current index, suggested gallery, estimated reduction).

**Benefits at a glance**
* **Visitor:** timely, low-friction hint; you stay in control — take it or skip it.
* **Venue/ops:** smoother flows with minimal data; consistent, auditable nudges.
* **IT/Security/Privacy:** no profiles in storage; short-lived, signed envelopes.
* **Experience:** shorter queues, calmer galleries, happier guests.
...
-------------------------
We convert a few *local* gallery signals into a **HOT insight** the UI can act on —
“show a comfort banner at MuseoCity; suggest a calmer nearby gallery” —
**without sharing raw data**. A simple **comfortIndex** is computed from
`crowdLevel` and `waitMinutes`. If the current selection exceeds the policy threshold,
and a nearby alternative is calmer (and within threshold), we emit a minimal,
**signed envelope** (audience, allowed use, expiry, compact assertions).
No personal profile or traces leave the device.
...
1) **Vocabulary** – Turtle with `@prefix` terms (e.g., `ex:crowdLevel`, `ex:comfortIndex`).
...
------------------------------
Inputs (local): per-gallery `crowdLevel` (0–10), `waitMinutes`, and optional `distanceMeters`
for alternatives; policy `comfortThreshold` and `maxSuggestedWalkM`.
Computation: `comfortIndex = (crowdLevel × 10) + waitMinutes`.
Outputs (shared): signed **envelope** with `audience`, `allowedUse`, `issuedAt`,
`expiry`, and **assertions**: `showComfortBanner`, optional `suggestGallery`,
`comfortIndex` (current), and `estimatedComfortReduction` (current − alternative).
...
Run
---
$ python3 museum_comfort_insight.py
"""
...
# CLASSES
ex:Venue     a rdfs:Class ; rdfs:label "Venue" ; rdfs:comment "A museum or site." .
ex:Gallery   a rdfs:Class ; rdfs:label "Gallery" ; rdfs:comment "Gallery/room tile." .
ex:ScanEvent a rdfs:Class ; rdfs:label "ScanEvent" ; rdfs:comment "Local tap/select event." .
...
# LINKS
ex:atVenue a owl:ObjectProperty ; rdfs:domain ex:ScanEvent ; rdfs:range ex:Venue .
ex:selectedGallery a owl:ObjectProperty ; rdfs:domain ex:ScanEvent ; rdfs:range ex:Gallery .
ex:atTime  a owl:DatatypeProperty ; rdfs:domain ex:ScanEvent ; rdfs:range xsd:dateTime .

# GALLERY DATA
ex:crowdLevel   a owl:DatatypeProperty ; rdfs:domain ex:Gallery ; rdfs:range xsd:decimal ;
  rdfs:comment "0..10 crowd index." .
ex:waitMinutes  a owl:DatatypeProperty ; rdfs:domain ex:Gallery ; rdfs:range xsd:decimal .
ex:distanceMeters a owl:DatatypeProperty ; rdfs:domain ex:Gallery ; rdfs:range xsd:decimal .

# DERIVED
ex:comfortIndex a owl:DatatypeProperty ; rdfs:domain ex:Gallery ; rdfs:range xsd:decimal ;
  rdfs:comment "(crowdLevel × 10) + waitMinutes" .

# INSIGHT OUTPUT
ex:showComfortBanner a owl:ObjectProperty ; rdfs:domain ex:Insight ; rdfs:range ex:Venue .
ex:suggestGallery    a owl:ObjectProperty ; rdfs:domain ex:Insight ; rdfs:range ex:Gallery .
ex:estimatedComfortReduction a owl:DatatypeProperty ; rdfs:domain ex:Insight ; rdfs:range xsd:decimal .

# POLICY
ex:comfortThreshold  a owl:DatatypeProperty ; rdfs:domain ex:Policy ; rdfs:range xsd:decimal .
ex:maxSuggestedWalkM a owl:DatatypeProperty ; rdfs:domain ex:Policy ; rdfs:range xsd:decimal .
...
# Venue and two galleries (selected + alternative)
ex:MuseoCity rdf:type ex:Venue .

ex:galleryImpressionists rdf:type ex:Gallery .
ex:galleryImpressionists ex:crowdLevel 7 .
ex:galleryImpressionists ex:waitMinutes 15 .
ex:galleryImpressionists ex:distanceMeters 0 .

ex:gallerySculpture rdf:type ex:Gallery .
ex:gallerySculpture ex:crowdLevel 3 .
ex:gallerySculpture ex:waitMinutes 5 .
ex:gallerySculpture ex:distanceMeters 80 .

# Scan context at MuseoCity (deterministic timestamp)
ex:scan1 rdf:type ex:ScanEvent .
ex:scan1 ex:atVenue ex:MuseoCity .
ex:scan1 ex:atTime "{ts}"^^xsd:dateTime .
ex:scan1 ex:selectedGallery ex:galleryImpressionists .

# Policy thresholds
ex:policy ex:comfortThreshold 70.0 .
ex:policy ex:maxSuggestedWalkM 200 .
"""
...
# @name: R1_ComfortIndexForAnyGallery
{
  ?G  rdf:type ex:Gallery .
  ?G  ex:crowdLevel ?C .
  ( ?C 10 ) math:product ?C10 .
  ?G  ex:waitMinutes ?W .
  ( ?C10 ?W ) math:sum ?Idx .
}
=>
{
  ?G ex:comfortIndex ?Idx .
} .

# @name: R2_BannerIfOverThreshold
{
  ex:scan1 ex:selectedGallery ?G1 .
  ?G1 ex:comfortIndex ?I1 .
  ex:policy ex:comfortThreshold ?T .
  ?I1 math:greaterThan ?T .
  ex:scan1 ex:atVenue ?Venue .
}
=>
{
  ex:insight1 rdf:type ex:Insight .
  ex:insight1 ex:showComfortBanner ?Venue .
  ex:insight1 ex:comfortIndex ?I1 .
} .

# @name: R3_SuggestAlternativeIfCalmerAndNearby
{
  ex:scan1 ex:selectedGallery ?G1 .
  ?G1 ex:comfortIndex ?I1 .

  ex:gallerySculpture ex:comfortIndex ?I2 .
  ?I1 math:greaterThan ?I2 .

  ex:gallerySculpture ex:distanceMeters ?D .
  ex:policy ex:maxSuggestedWalkM ?Max .
  ?D math:notGreaterThan ?Max .
}
=>
{
  ex:insight1 ex:suggestGallery ex:gallerySculpture .
} .

# @name: R4_ComputeComfortReductionAndGuardAlt
{
  ex:scan1 ex:selectedGallery ?G1 .
  ?G1 ex:comfortIndex ?I1 .
  ex:gallerySculpture ex:comfortIndex ?I2 .
  ( ?I1 ?I2 ) math:difference ?Reduction .

  ex:policy ex:comfortThreshold ?T .
  ?I2 math:notGreaterThan ?T .
}
=>
{
  ex:insight1 ex:estimatedComfortReduction ?Reduction .
} .
"""
...
def synthesize_if_missing(g: Graph) -> None:
    ...
    try:
        C1 = num(r'ex:galleryImpressionists\s+ex:crowdLevel\s+([0-9.]+)\s*\.')
        W1 = num(r'ex:galleryImpressionists\s+ex:waitMinutes\s+([0-9.]+)\s*\.')
        C2 = num(r'ex:gallerySculpture\s+ex:crowdLevel\s+([0-9.]+)\s*\.')
        W2 = num(r'ex:gallerySculpture\s+ex:waitMinutes\s+([0-9.]+)\s*\.')
        D2 = num(r'ex:gallerySculpture\s+ex:distanceMeters\s+([0-9.]+)\s*\.')
        T  = num(r'ex:policy\s+ex:comfortThreshold\s+([0-9.]+)\s*\.')
        Mx = num(r'ex:policy\s+ex:maxSuggestedWalkM\s+([0-9.]+)\s*\.')
    except Exception:
        return

    I1 = (C1 * 10) + W1
    I2 = (C2 * 10) + W2
    g.add("ex:galleryImpressionists","ex:comfortIndex",I1)
    g.add("ex:gallerySculpture","ex:comfortIndex",I2)

    if I1 > T:
        g.add("ex:insight1","rdf:type","ex:Insight")
        g.add("ex:insight1","ex:showComfortBanner","ex:MuseoCity")
        g.add("ex:insight1","ex:comfortIndex",I1)
        if I1 > I2 and D2 <= Mx:
            g.add("ex:insight1","ex:suggestGallery","ex:gallerySculpture")
            if I2 <= T:
                g.add("ex:insight1","ex:estimatedComfortReduction", I1 - I2)
...
ALLOWED_USES = {"ui.museum.comfort-banner"}
SENSITIVE_KEYS = {"userId","deviceId","profileId","locationTrace","beaconIds"}
...
def envelope_from_graph(g: Graph) -> Dict[str, Any]:
    venue=None; idx=None; sugg=None; reduct=None
    for (s,p,o) in g.triples():
        if s=="ex:insight1" and p=="ex:showComfortBanner": venue=o
        if s=="ex:insight1" and p=="ex:comfortIndex": idx=o
        if s=="ex:insight1" and p=="ex:suggestGallery": sugg=o
        if s=="ex:insight1" and p=="ex:estimatedComfortReduction": reduct=o
    assertions=[]
    if venue: assertions.append({"type":"ex:showComfortBanner","value":True})
    if sugg:  assertions.append({"type":"ex:suggestGallery","value":sugg})
    if idx is not None: assertions.append({"type":"ex:comfortIndex","value":float(idx)})
    if reduct is not None: assertions.append({"type":"ex:estimatedComfortReduction","value":float(reduct)})
    env = {
        "@type":"ex:Envelope",
        "ex:audience": venue or "ex:MuseoCity",
        "ex:allowedUse":"ui.museum.comfort-banner",
...
def build_compact_reason(g: Graph, env: Dict[str, Any]) -> Dict[str, Any]:
    C1 = gnum(g,"ex:galleryImpressionists","ex:crowdLevel")
    W1 = gnum(g,"ex:galleryImpressionists","ex:waitMinutes")
    I1 = gnum(g,"ex:galleryImpressionists","ex:comfortIndex")

    C2 = gnum(g,"ex:gallerySculpture","ex:crowdLevel")
    W2 = gnum(g,"ex:gallerySculpture","ex:waitMinutes")
    D2 = gnum(g,"ex:gallerySculpture","ex:distanceMeters")
    I2 = gnum(g,"ex:gallerySculpture","ex:comfortIndex")

    T  = gnum(g,"ex:policy","ex:comfortThreshold")
    Mx = gnum(g,"ex:policy","ex:maxSuggestedWalkM")

    reduction = gnum(g,"ex:insight1","ex:estimatedComfortReduction")
    sugg = gstr(g,"ex:insight1","ex:suggestGallery")
    venue = gstr(g,"ex:insight1","ex:showComfortBanner") or env.get("ex:audience")

    return {
        "inputs": {
            "audience": venue,
            "selected": {"crowdLevel": C1, "waitMinutes": W1},
            "alternative": {"crowdLevel": C2, "waitMinutes": W2, "distanceMeters": D2},
            "policy": {"comfortThreshold": T, "maxSuggestedWalkM": Mx}
        },
        "calculation": {
            "comfortIndex": {"selected": I1, "alternative": I2},
            "estimatedComfortReduction": reduction
        },
        "decisions": [
            f"Show comfort banner at {venue} because {I1:.2f} > threshold {T:.2f}.",
            f"Suggest {sugg} because current {I1:.2f} > alt {I2:.2f} and distance {D2:.0f} m ≤ {Mx:.0f} m.",
            f"Estimated comfort reduction ≈ {reduction:.2f} points."
        ]
    }
...
def print_reason_compact(reason: Dict[str, Any], g: Graph) -> None:
    ...
    print(f"  • Selected gallery: crowd {sel['crowdLevel']}/10; wait {sel['waitMinutes']} min")
    print(f"  • Alternative:      crowd {alt['crowdLevel']}/10; wait {alt['waitMinutes']} min; distance {alt['distanceMeters']} m")
    print(f"  • Policy: threshold {pol['comfortThreshold']}, max walk {pol['maxSuggestedWalkM']} m")
    print("\nComputation:")
    print(f"  • comfortIndex(selected)   = {calc['comfortIndex']['selected']}")
    print(f"  • comfortIndex(alternative)= {calc['comfortIndex']['alternative']}")
    if calc['estimatedComfortReduction'] is not None:
        print(f"  • estimated comfort reduction = {calc['estimatedComfortReduction']}")
    print("\nDecisions:")
    for d in dec: print(f"  • {d}")
    print("\nDerived facts (key subset):")
    focus={"ex:comfortIndex","ex:showComfortBanner","ex:suggestGallery","ex:estimatedComfortReduction"}
    for (s,p,o) in g.triples():
...
def check_audience(env: Dict[str, Any]) -> Tuple[bool,str]:
    return (env.get("ex:audience")=="ex:MuseoCity", "audience matches")
...
def check_better(g: Graph) -> Tuple[bool,str]:
    r: Dict[str,float]={}
    for (s,p,o) in g.triples():
        if p=="ex:comfortIndex": r[s]=float(o)
    cur=None; alt="ex:gallerySculpture"
    for (s,p,o) in g.triples():
        if s=="ex:scan1" and p=="ex:selectedGallery": cur=o
    if alt not in r or cur not in r: return False, "missing comfortIndex"
    return (r[cur]>r[alt], f"alternative calmer: {r[cur]:.2f} → {r[alt]:.2f}")
...
def main() -> None:
    g, env, reason = interaction_flow("ex:MuseoCity")
    print_answer(env)

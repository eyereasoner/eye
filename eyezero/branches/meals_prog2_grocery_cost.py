...
Overview:
  Consume ./branches/bus/meal/meal_plan.json, compare with pantry and a simple price list, and generate
  a deduplicated grocery list with totals. Prints Answer/Reason/Check and writes
  ./branches/bus/meal/grocery_list.json for Program 3.

Run:
  python meals_prog2_grocery_cost.py --in ./branches/bus/meal/meal_plan.json --out ./branches/bus/meal/grocery_list.json
"""
...
def main():
    ...
    ap.add_argument("--in", dest="infile", default="./branches/bus/meal/meal_plan.json")
    ap.add_argument("--out", default="./branches/bus/meal/grocery_list.json")
    args = ap.parse_args()

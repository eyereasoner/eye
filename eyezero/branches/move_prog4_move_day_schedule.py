...
Overview:
  - Reads './branches/bus/move/van_plan.json' and './branches/bus/move/packing_plan.json'.
  - Creates a day timeline: load → drive → unload per trip.
  - Emits a minimal ICS-like text plus a JSON artifact './branches/bus/move/move_schedule.json'.
...
def main():
    ...
    ap.add_argument("--in_van", default="./branches/bus/move/van_plan.json")
    ap.add_argument("--in_packing", default="./branches/bus/move/packing_plan.json")
    ap.add_argument("--out", default="./branches/bus/move/move_schedule.json")
    args = ap.parse_args()
    ...
    with open("./branches/bus/move/move_schedule.ics.txt","w",encoding="utf-8") as f: f.write(sched["ics_text"])
    print("\nWrote ./branches/bus/move/move_schedule.json and ./branches/bus/move/move_schedule.ics.txt")

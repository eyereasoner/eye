...
# -*- coding: utf-8 -*-
"""
Vaccine Thaw Insight — Compact, EYE-compatible demonstrator
===========================================================

STORY
-----
Late afternoon at **Clinic B**. Li opens the vaccine tray and the tablet beside
it already knows a few harmless bits of context: **thawed doses**, **expected
arrivals**, **minutes since thaw**, and the product’s **remaining shelf life**.
It does a tiny, local calculation — no EHR dump, no patient data — a quick
**waste-risk index**:

    riskIndex = (thawedDoses − expectedArrivals) × (minutesSinceThaw ÷ shelfLifeMinutes)

If the current plan’s risk index exceeds a policy threshold, the app shows a
gentle banner and suggests a **micro-thaw plan** (thaw fewer doses later). Li
checks staffing — safe — switches the plan, and the banner fades. What leaves
the device isn’t any patient identifier or dose trace — just a signed,
audience-scoped **insight envelope**: who it’s for (Clinic B), what it allows
(show a thaw banner and suggestion), when it expires, and a compact set of
assertions (current risk, suggested plan, estimated reduction).

**Benefits at a glance**
* **Nurse/tech (Li):** timely, optional hint; you stay in control — adjust only if safe.
* **Operations:** fewer discarded doses; consistent, auditable nudges with minimal data.
* **IT/Security/Privacy:** no PHI; short-lived, signed envelopes; standards-based vocab/SHACL/N3.
* **Experience & cost:** better match of supply to arrivals; less waste.
...
-------------------------
We convert a few *local* vaccination-station signals into a **HOT insight** the UI
can act on — “show thaw banner at Clinic B; suggest micro-thaw plan” —
**without sharing raw data**. A simple **riskIndex** is computed from
`thawedDoses`, `expectedArrivals`, `minutesSinceThaw`, and `shelfLifeMinutes`.
If the current plan exceeds policy, and the micro-thaw plan is under threshold
and respects a **minimum ready-dose** guard, we emit a minimal, **signed
envelope** (audience, allowed use, expiry, compact assertions). No patient or
staff identifiers leave the device.
...
1) **Vocabulary** – Turtle with `@prefix` terms (e.g., `ex:thawedDoses`, `ex:riskIndex`).
...
------------------------------
Inputs (local): per-plan `thawedDoses`, `expectedArrivals`, `minutesSinceThaw`,
`shelfLifeMinutes`; policy `riskThreshold` and `minReadyDoses`.  
Computation: `riskIndex = (thawedDoses − expectedArrivals) × (minutesSinceThaw ÷ shelfLifeMinutes)`.  
Outputs (shared): signed **envelope** with `audience`, `allowedUse`, `issuedAt`,
`expiry`, and **assertions**: `showThawBanner`, optional `suggestPlan`,
`riskIndex` (current), and `estimatedRiskReduction` (current − alternative).
...
Run
---
$ python3 vaccine_thaw_insight.py
"""
...
# CLASSES
ex:Clinic    a rdfs:Class ; rdfs:label "Clinic" ;   rdfs:comment "A clinic or vaccination site." .
ex:Plan      a rdfs:Class ; rdfs:label "Plan" ;     rdfs:comment "Thaw/dose plan." .
ex:ScanEvent a rdfs:Class ; rdfs:label "ScanEvent"; rdfs:comment "Local selection/scan event." .
ex:Policy    a rdfs:Class ; rdfs:label "Policy" ;  rdfs:comment "Local thresholds and guards." .
...
# PLAN DATA
ex:thawedDoses       a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .
ex:expectedArrivals  a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .
ex:minutesSinceThaw  a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .
ex:shelfLifeMinutes  a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .

# DERIVED
ex:riskIndex         a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal ;
  rdfs:comment "(thawedDoses − expectedArrivals) × (minutesSinceThaw ÷ shelfLifeMinutes)" .
ex:doseBuffer        a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .
ex:timeRemainingMin  a owl:DatatypeProperty ; rdfs:domain ex:Plan ; rdfs:range xsd:decimal .

# INSIGHT OUTPUT
ex:showThawBanner    a owl:ObjectProperty ; rdfs:domain ex:Insight ; rdfs:range ex:Clinic .
ex:suggestPlan       a owl:ObjectProperty ; rdfs:domain ex:Insight ; rdfs:range ex:Plan .
ex:estimatedRiskReduction a owl:DatatypeProperty ; rdfs:domain ex:Insight ; rdfs:range xsd:decimal .

# POLICY
ex:riskThreshold     a owl:DatatypeProperty ; rdfs:domain ex:Policy ; rdfs:range xsd:decimal .
ex:minReadyDoses     a owl:DatatypeProperty ; rdfs:domain ex:Policy ; rdfs:range xsd:decimal .
...
# Clinic and two plans (current + micro-thaw)
ex:ClinicB rdf:type ex:Clinic .

ex:currentPlan rdf:type ex:Plan .
ex:currentPlan ex:thawedDoses 40 .
ex:currentPlan ex:expectedArrivals 30 .
ex:currentPlan ex:minutesSinceThaw 60 .
ex:currentPlan ex:shelfLifeMinutes 360 .

ex:microThaw rdf:type ex:Plan .
ex:microThaw ex:thawedDoses 30 .
ex:microThaw ex:expectedArrivals 30 .
ex:microThaw ex:minutesSinceThaw 30 .
ex:microThaw ex:shelfLifeMinutes 360 .

# Scan context at ClinicB (deterministic timestamp)
ex:scan1 rdf:type ex:ScanEvent .
ex:scan1 ex:atClinic ex:ClinicB .
ex:scan1 ex:atTime "{ts}"^^xsd:dateTime .
ex:scan1 ex:usesPlan ex:currentPlan .

# Policy thresholds/guards
ex:policy ex:riskThreshold 1.0 .
ex:policy ex:minReadyDoses 20 .
"""
...
# @name: R1_RiskIndexForAnyPlan
{
  ?P  rdf:type ex:Plan .
  ?P  ex:thawedDoses ?Td .
  ?P  ex:expectedArrivals ?Exp .
  ( ?Td ?Exp ) math:difference ?Excess .
  ?P  ex:minutesSinceThaw ?Min .
  ?P  ex:shelfLifeMinutes ?Life .
  ( ?Min ?Life ) math:quotient ?Frac .
  ( ?Excess ?Frac ) math:product ?Risk .
}
=>
{
  ?P ex:riskIndex ?Risk .
...
# @name: R2_BannerIfOverThreshold (self-contained)
{
  ...
  ex:policy ex:riskThreshold ?T .
  ?R1 math:greaterThan ?T .
...
=>
{
  ...
  ex:insight1 ex:showThawBanner ?Clinic .
  ex:insight1 ex:riskIndex ?R1 .
} .

# @name: R3_SuggestMicroThawIfBetter (self-contained)
{
  ex:scan1 ex:usesPlan ?P1 .
  ?P1 ex:riskIndex ?R1 .

  ex:microThaw ex:riskIndex ?R2 .
  ?R1 math:greaterThan ?R2 .
}
=>
{
  ex:insight1 ex:suggestPlan ex:microThaw .
} .

# @name: R4_ComputeRiskReductionAndGuardAlt (self-contained)
{
  ex:scan1 ex:usesPlan ?P1 .
  ?P1 ex:riskIndex ?R1 .
  ex:microThaw ex:riskIndex ?R2 .
  ( ?R1 ?R2 ) math:difference ?Reduction .

  ex:policy ex:riskThreshold ?T .
  ?R2 math:notGreaterThan ?T .

  ex:policy ex:minReadyDoses ?MinReady .
  ex:microThaw ex:thawedDoses ?Td2 .
  ?Td2 math:notLessThan ?MinReady .
}
=>
{
  ex:insight1 ex:estimatedRiskReduction ?Reduction .
} .

# @name: R5_OptionalBuffers (dose buffer and time remaining per plan)
{
  ?P  rdf:type ex:Plan .
  ?P  ex:thawedDoses ?Td .
  ?P  ex:expectedArrivals ?Exp .
  ( ?Td ?Exp ) math:difference ?Buf .
  ?Buf math:notLessThan 0 .

  ?P  ex:shelfLifeMinutes ?Life .
  ?P  ex:minutesSinceThaw ?Min .
  ( ?Life ?Min ) math:difference ?Remain .
  ?Remain math:notLessThan 0 .
}
=>
{
  ?P ex:doseBuffer ?Buf .
  ?P ex:timeRemainingMin ?Remain .
} .
"""
...
def synthesize_if_missing(g: Graph) -> None:
    ...
    try:
        Td1 = num(r'ex:currentPlan\s+ex:thawedDoses\s+([0-9.]+)\s*\.')
        Ea1 = num(r'ex:currentPlan\s+ex:expectedArrivals\s+([0-9.]+)\s*\.')
        Mn1 = num(r'ex:currentPlan\s+ex:minutesSinceThaw\s+([0-9.]+)\s*\.')
        Lf1 = num(r'ex:currentPlan\s+ex:shelfLifeMinutes\s+([0-9.]+)\s*\.')
        Td2 = num(r'ex:microThaw\s+ex:thawedDoses\s+([0-9.]+)\s*\.')
        Ea2 = num(r'ex:microThaw\s+ex:expectedArrivals\s+([0-9.]+)\s*\.')
        Mn2 = num(r'ex:microThaw\s+ex:minutesSinceThaw\s+([0-9.]+)\s*\.')
        Lf2 = num(r'ex:microThaw\s+ex:shelfLifeMinutes\s+([0-9.]+)\s*\.')
        T   = num(r'ex:policy\s+ex:riskThreshold\s+([0-9.]+)\s*\.')
        MinReady = num(r'ex:policy\s+ex:minReadyDoses\s+([0-9.]+)\s*\.')
    except Exception:
        return

    R1 = (Td1 - Ea1) * (Mn1 / Lf1)
    R2 = (Td2 - Ea2) * (Mn2 / Lf2)
    g.add("ex:currentPlan","ex:riskIndex",R1)
    g.add("ex:microThaw","ex:riskIndex",R2)
    ...
    rem1 = Lf1 - Mn1
    rem2 = Lf2 - Mn2
    if buf1 >= 0: g.add("ex:currentPlan","ex:doseBuffer",buf1)
    if buf2 >= 0: g.add("ex:microThaw","ex:doseBuffer",buf2)
    if rem1 >= 0: g.add("ex:currentPlan","ex:timeRemainingMin",rem1)
    if rem2 >= 0: g.add("ex:microThaw","ex:timeRemainingMin",rem2)

    if R1 > T:
        g.add("ex:insight1","rdf:type","ex:Insight")
        g.add("ex:insight1","ex:showThawBanner","ex:ClinicB")
        g.add("ex:insight1","ex:riskIndex",R1)
        if (R1 > R2) and (R2 <= T) and (Td2 >= MinReady):
            g.add("ex:insight1","ex:suggestPlan","ex:microThaw")
            g.add("ex:insight1","ex:estimatedRiskReduction",R1 - R2)
...
ALLOWED_USES = {"ui.clinic.thaw-banner"}
SENSITIVE_KEYS = {"patientId","mrn","name","dob","staffId","shiftRoster","doseTrace","gpsTrace"}
...
def envelope_from_graph(g: Graph) -> Dict[str, Any]:
    clinic=None; risk=None; sugg=None; reduct=None
    for (s,p,o) in g.triples():
        if s=="ex:insight1" and p=="ex:showThawBanner":         clinic=o
        if s=="ex:insight1" and p=="ex:riskIndex":              risk=o
        if s=="ex:insight1" and p=="ex:suggestPlan":            sugg=o
        if s=="ex:insight1" and p=="ex:estimatedRiskReduction": reduct=o
    assertions=[]
    if clinic: assertions.append({"type":"ex:showThawBanner","value":True})
    if sugg:   assertions.append({"type":"ex:suggestPlan","value":sugg})
    if risk is not None:   assertions.append({"type":"ex:riskIndex","value":float(risk)})
    if reduct is not None: assertions.append({"type":"ex:estimatedRiskReduction","value":float(reduct)})
    env = {
        "@type":"ex:Envelope",
        "ex:audience": clinic or "ex:ClinicB",
        "ex:allowedUse":"ui.clinic.thaw-banner",
...
def build_compact_reason(g: Graph, env: Dict[str, Any]) -> Dict[str, Any]:
    Td1 = gnum(g,"ex:currentPlan","ex:thawedDoses"); Ea1 = gnum(g,"ex:currentPlan","ex:expectedArrivals")
    Mn1 = gnum(g,"ex:currentPlan","ex:minutesSinceThaw"); Lf1 = gnum(g,"ex:currentPlan","ex:shelfLifeMinutes")
    Td2 = gnum(g,"ex:microThaw","ex:thawedDoses");       Ea2 = gnum(g,"ex:microThaw","ex:expectedArrivals")
    Mn2 = gnum(g,"ex:microThaw","ex:minutesSinceThaw");  Lf2 = gnum(g,"ex:microThaw","ex:shelfLifeMinutes")
    R1  = gnum(g,"ex:currentPlan","ex:riskIndex");       R2  = gnum(g,"ex:microThaw","ex:riskIndex")
    B1  = gnum(g,"ex:currentPlan","ex:doseBuffer");      B2  = gnum(g,"ex:microThaw","ex:doseBuffer")
    Tm1 = gnum(g,"ex:currentPlan","ex:timeRemainingMin");Tm2 = gnum(g,"ex:microThaw","ex:timeRemainingMin")
    T   = gnum(g,"ex:policy","ex:riskThreshold");        MinReady = gnum(g,"ex:policy","ex:minReadyDoses")
    suggested = gstr(g,"ex:insight1","ex:suggestPlan")
    clinic = gstr(g,"ex:insight1","ex:showThawBanner") or env.get("ex:audience")
    reduction = gnum(g,"ex:insight1","ex:estimatedRiskReduction")

    return {
        "inputs": {
            "audience": clinic,
            "currentPlan": {"thawedDoses": Td1, "expectedArrivals": Ea1, "minutesSinceThaw": Mn1, "shelfLifeMinutes": Lf1},
            "microThaw":   {"thawedDoses": Td2, "expectedArrivals": Ea2, "minutesSinceThaw": Mn2, "shelfLifeMinutes": Lf2},
            "policy": {"riskThreshold": T, "minReadyDoses": MinReady}
        },
        "calculation": {
            "riskIndex": {"current": R1, "microThaw": R2},
            "doseBuffer": {"current": B1, "microThaw": B2},
            "timeRemainingMin": {"current": Tm1, "microThaw": Tm2},
            "estimatedRiskReduction": reduction
        },
        "decisions": [
            f"Show thaw banner at {clinic} because {R1:.2f} > threshold {T:.2f}.",
            f"Suggest {suggested} because current {R1:.2f} > microThaw {R2:.2f} and thawed {Td2:.0f} ≥ min ready {MinReady:.0f}.",
            f"Estimated risk reduction ≈ {reduction:.2f}."
        ]
    }
...
def print_reason_compact(reason: Dict[str, Any], g: Graph) -> None:
    ...
    cp = inp["currentPlan"]; mp = inp["microThaw"]; pol = inp["policy"]
    print(f"  • Current plan: thawed {cp['thawedDoses']}; expected {cp['expectedArrivals']}; since thaw {cp['minutesSinceThaw']} min; shelf life {cp['shelfLifeMinutes']} min")
    print(f"  • Micro-thaw:   thawed {mp['thawedDoses']}; expected {mp['expectedArrivals']}; since thaw {mp['minutesSinceThaw']} min; shelf life {mp['shelfLifeMinutes']} min")
    print(f"  • Policy: risk threshold {pol['riskThreshold']}; min ready doses {pol['minReadyDoses']}")
    print("\nComputation:")
    print(f"  • riskIndex(current)  = {calc['riskIndex']['current']}")
    print(f"  • riskIndex(microThaw)= {calc['riskIndex']['microThaw']}")
    if calc['doseBuffer']['current'] is not None or calc['doseBuffer']['microThaw'] is not None:
        print(f"  • doseBuffer(current)  = {calc['doseBuffer']['current']}")
        print(f"  • doseBuffer(microThaw)= {calc['doseBuffer']['microThaw']}")
    if calc['timeRemainingMin']['current'] is not None or calc['timeRemainingMin']['microThaw'] is not None:
        print(f"  • timeRemaining(current)  = {calc['timeRemainingMin']['current']} min")
        print(f"  • timeRemaining(microThaw)= {calc['timeRemainingMin']['microThaw']} min")
    if calc['estimatedRiskReduction'] is not None:
        print(f"  • estimated risk reduction = {calc['estimatedRiskReduction']}")
    print("\nDecisions:")
    for d in dec: print(f"  • {d}")
    print("\nDerived facts (key subset):")
    focus={"ex:riskIndex","ex:doseBuffer","ex:timeRemainingMin","ex:showThawBanner","ex:suggestPlan","ex:estimatedRiskReduction"}
    for (s,p,o) in g.triples():
...
def check_audience(env: Dict[str, Any]) -> Tuple[bool,str]:
    return (env.get("ex:audience")=="ex:ClinicB", "audience matches")
...
def check_better(g: Graph) -> Tuple[bool,str]:
    ...
    if cur not in r or alt not in r: return False, "missing riskIndex"
    return (r[cur]>r[alt], f"microThaw lower risk: {r[cur]:.2f} → {r[alt]:.2f}")
...
def main() -> None:
    g, env, reason = interaction_flow("ex:ClinicB")
    print_answer(env)

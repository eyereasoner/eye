...
Overview (matches this chat):
  - Reads './branches/bus/energy/house_usage_sample.json' (or your file via --in).
  - Normalizes per-appliance usage to kWh/day.
  - Computes daily cost and daily CO₂.
  - Prints Answer/Reasons/Check; writes './branches/bus/energy/baseline_daily.json' for Program 2.
...
def main():
    ...
    ap.add_argument("--in", dest="infile", default="./branches/bus/energy/house_usage_sample.json")
    ap.add_argument("--out", default="./branches/bus/energy/baseline_daily.json")
    args = ap.parse_args()

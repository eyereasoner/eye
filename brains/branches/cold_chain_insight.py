...
-------------------------
We turn a few *local* cold-chain signals into a **HOT insight** the UI can act on—
“show a cold-chain warning at Facility Y; fast-track the pallet”—**without
sharing raw data**. A simple **riskIndex** is computed from ambient temperature,
setpoint, and exposure minutes. If the current stage’s risk exceeds policy, and
a fast track keeps risk lower, we emit a minimal, **signed envelope** (audience,
allowed use, expiry, compact assertions). The raw temps/timestamps stay local.
...
1) **Vocabulary** – Turtle with `@prefix` terms (e.g., `ex:ambientC`, `ex:riskIndex`).
...
------------------------------
Inputs (local): `setpointC` (target temp), per-stage `ambientC` and `exposureMinutes`,
and a policy `riskThreshold`.  
Computation: `riskIndex = (ambientC - setpointC) * exposureMinutes`.  
Outputs (shared): signed **envelope** with `audience`, `allowedUse`, `issuedAt`,
`expiry`, and **assertions**: `showColdBanner`, optional `suggestStage`,
...
# Facility, pallet, and two stages (current + fast track)
ex:FacilityY rdf:type ex:Facility .

ex:pallet1 rdf:type ex:Pallet .
ex:pallet1 ex:setpointC 4.0 .

ex:currentStage rdf:type ex:Stage .
ex:currentStage ex:ambientC 9.0 .
ex:currentStage ex:exposureMinutes 35 .

ex:fastTrack rdf:type ex:Stage .
ex:fastTrack ex:ambientC 6.0 .
ex:fastTrack ex:exposureMinutes 10 .

# Scan context at FacilityY (deterministic timestamp)
ex:scan1 rdf:type ex:ScanEvent .
ex:scan1 ex:atFacility ex:FacilityY .
ex:scan1 ex:atTime "{ts}"^^xsd:dateTime .
ex:scan1 ex:usesStage ex:currentStage .
ex:scan1 ex:forPallet ex:pallet1 .

# Policy threshold for triggering cold-chain banner (risk units)
ex:policy ex:riskThreshold 25.0 .
"""
...
# @name: R1_RiskIndexForAnyStage
{
  ex:scan1 ex:forPallet ?P .
  ?P       ex:setpointC ?Sp .

  ?S  rdf:type ex:Stage .
  ?S  ex:ambientC ?Amb .
  ?S  ex:exposureMinutes ?Min .

  ( ?Amb ?Sp ) math:difference ?DeltaC .
  ( ?DeltaC ?Min ) math:product ?Risk .
}
=>
{
  ?S ex:riskIndex ?Risk .
...
{
  ex:scan1 ex:usesStage ?S1 .
  ?S1 ex:riskIndex ?R1 .

  ex:policy ex:riskThreshold ?T .
  ?R1 math:greaterThan ?T .

  ex:scan1 ex:atFacility ?Fac .
}
=>
{
  ex:insight1 rdf:type ex:Insight .
  ex:insight1 ex:showColdBanner ?Fac .
  ex:insight1 ex:riskIndex ?R1 .
} .

# @name: R3_SuggestFastTrackIfBetter (self-contained)
{
  ex:scan1 ex:usesStage ?S1 .
  ?S1 ex:riskIndex ?R1 .

  ex:fastTrack ex:riskIndex ?R2 .
  ?R1 math:greaterThan ?R2 .
}
=>
{
  ex:insight1 ex:suggestStage ex:fastTrack .
} .
...
# @name: R5_OptionalBufferMinutes (how many minutes until threshold at this ambient)
{
  ex:policy ex:riskThreshold ?T .
  ex:scan1 ex:forPallet ?P .
  ?P       ex:setpointC ?Sp .
  ...
  ( ?Amb ?Sp ) math:difference ?DeltaC .
  ( ?T ?DeltaC ) math:quotient ?MaxMinAtAmbient .
  ( ?MaxMinAtAmbient ?Min ) math:difference ?Buffer .

  ?Buffer math:notLessThan 0 .
}
=>
{
...
def synthesize_if_missing(g: Graph) -> None:
    ...
    try:
        Sp = num(r'ex:pallet1\s+ex:setpointC\s+([0-9.]+)\s*\.')
        A1 = num(r'ex:currentStage\s+ex:ambientC\s+([0-9.]+)\s*\.')
        M1 = num(r'ex:currentStage\s+ex:exposureMinutes\s+([0-9.]+)\s*\.')
        A2 = num(r'ex:fastTrack\s+ex:ambientC\s+([0-9.]+)\s*\.')
        M2 = num(r'ex:fastTrack\s+ex:exposureMinutes\s+([0-9.]+)\s*\.')
    ...
    R1 = (A1 - Sp) * M1
    R2 = (A2 - Sp) * M2
    g.add("ex:currentStage","ex:riskIndex",R1)
    g.add("ex:fastTrack","ex:riskIndex",R2)

    # Optional buffer minutes per stage
    if (A1 - Sp) != 0:
        maxMin1 = T / (A1 - Sp)
        buf1 = maxMin1 - M1
        if buf1 >= 0: g.add("ex:currentStage","ex:bufferMinutes",buf1)
    if (A2 - Sp) != 0:
        maxMin2 = T / (A2 - Sp)
        buf2 = maxMin2 - M2
        if buf2 >= 0: g.add("ex:fastTrack","ex:bufferMinutes",buf2)

    if R1 > T:
        g.add("ex:insight1","rdf:type","ex:Insight")
        g.add("ex:insight1","ex:showColdBanner","ex:FacilityY")
        g.add("ex:insight1","ex:riskIndex",R1)
        if R1 > R2:
            g.add("ex:insight1","ex:suggestStage","ex:fastTrack")
            if R2 <= T:
...
ALLOWED_USES = {"ui.coldchain.banner"}
SENSITIVE_KEYS = {"gpsTrace","payloadKg","routeGeometry","driverId","DriverID",
                  "customerId","orderId","temperatureSeries"}
...
def envelope_from_graph(g: Graph) -> Dict[str, Any]:
    fac=None; risk=None; sugg=None; reduct=None
    for (s,p,o) in g.triples():
        if s=="ex:insight1" and p=="ex:showColdBanner":        fac=o
        if s=="ex:insight1" and p=="ex:riskIndex":             risk=o
        if s=="ex:insight1" and p=="ex:suggestStage":          sugg=o
        if s=="ex:insight1" and p=="ex:estimatedRiskReduction": reduct=o
    assertions=[]
    if fac: assertions.append({"type":"ex:showColdBanner","value":True})
    if sugg: assertions.append({"type":"ex:suggestStage","value":sugg})
    ...
    env = {
        "@type":"ex:Envelope",
        "ex:audience": fac or "ex:FacilityY",
        "ex:allowedUse":"ui.coldchain.banner",
...
def build_compact_reason(g: Graph, env: Dict[str, Any]) -> Dict[str, Any]:
    Sp = gnum(g,"ex:pallet1","ex:setpointC")
    A1 = gnum(g,"ex:currentStage","ex:ambientC"); M1 = gnum(g,"ex:currentStage","ex:exposureMinutes")
    A2 = gnum(g,"ex:fastTrack","ex:ambientC");   M2 = gnum(g,"ex:fastTrack","ex:exposureMinutes")
    T  = gnum(g,"ex:policy","ex:riskThreshold")
    R1 = gnum(g,"ex:currentStage","ex:riskIndex"); R2 = gnum(g,"ex:fastTrack","ex:riskIndex")
    B1 = gnum(g,"ex:currentStage","ex:bufferMinutes"); B2 = gnum(g,"ex:fastTrack","ex:bufferMinutes")
    suggested = gstr(g,"ex:insight1","ex:suggestStage")
    fac = gstr(g,"ex:insight1","ex:showColdBanner") or env.get("ex:audience")

    reduction = (R1 - R2) if (R1 is not None and R2 is not None) else None

    return {
        "inputs": {
            "audience": fac,
            "setpointC": Sp,
            "currentStage": {"ambientC": A1, "exposureMinutes": M1},
            "fastTrack":   {"ambientC": A2, "exposureMinutes": M2},
            "policyRiskThreshold": T
        },
        "calculation": {
            "riskIndex": {"current": R1, "fastTrack": R2},
            "bufferMinutes": {"current": B1, "fastTrack": B2},
            "estimatedRiskReduction": reduction
        },
        "decisions": [
            f"Show cold-chain banner at {fac} because {R1:.2f} > threshold {T:.2f}.",
            f"Suggest {suggested} because current {R1:.2f} > fastTrack {R2:.2f}.",
...
def print_reason_compact(reason: Dict[str, Any], g: Graph) -> None:
    ...
    print(f"  • Setpoint: {inp['setpointC']} °C")
    cs = inp["currentStage"]; ft = inp["fastTrack"]
    print(f"  • Current stage: ambient {cs['ambientC']} °C; exposure {cs['exposureMinutes']} min")
    print(f"  • Fast track:    ambient {ft['ambientC']} °C; exposure {ft['exposureMinutes']} min")
    print(f"  • Policy risk threshold: {inp['policyRiskThreshold']}")
    print("\nComputation:")
    print(f"  • riskIndex(current)  = {calc['riskIndex']['current']}")
    print(f"  • riskIndex(fastTrack)= {calc['riskIndex']['fastTrack']}")
    if calc['bufferMinutes']['current'] is not None or calc['bufferMinutes']['fastTrack'] is not None:
        print(f"  • bufferMinutes(current)  = {calc['bufferMinutes']['current']}")
        print(f"  • bufferMinutes(fastTrack)= {calc['bufferMinutes']['fastTrack']}")
    ...
    focus={"ex:riskIndex","ex:bufferMinutes","ex:showColdBanner","ex:suggestStage","ex:estimatedRiskReduction"}
    for (s,p,o) in g.triples():
...
def check_audience(env: Dict[str, Any]) -> Tuple[bool,str]:
    return (env.get("ex:audience")=="ex:FacilityY", "audience matches")
...
def check_better(g: Graph) -> Tuple[bool,str]:
    ...
    cur=None; alt=None
    for (s,p,o) in g.triples():
        if s=="ex:scan1" and p=="ex:usesStage": cur=o
        if s=="ex:insight1" and p=="ex:suggestStage": alt=o
    if alt is None: return False, "no suggested stage"
    if cur not in r or alt not in r: return False, "missing riskIndex"
    return (r[cur]>r[alt], f"fastTrack better: {r[cur]:.2f} → {r[alt]:.2f}")
...
def main() -> None:
    g, env, reason = interaction_flow("ex:FacilityY")
    print_answer(env)

...
Overview:
  Consume meal_./branches/bus/meal/plan.json and ./branches/bus/meal/grocery_list.json; compute per-day calories/macros from recipe
  nutrition and confirm ingredients are covered by pantry + purchases. Also output a simple
  cooking order. Prints Answer/Reason/Check and writes ./branches/bus/meal/final_week_plan.json.

Run:
  python meals_prog3_nutrition_schedule.py --plan ./branches/bus/meal/meal_plan.json --groceries ./branches/bus/meal/grocery_list.json
"""
...
def main():
    ...
    ap.add_argument("--plan", default="./branches/bus/meal/meal_plan.json")
    ap.add_argument("--groceries", default="./branches/bus/meal/grocery_list.json")
    args = ap.parse_args()
    ...
    with open("./branches/bus/meal/final_week_plan.json","w",encoding="utf-8") as f:
        json.dump({"summary": summary, "schedule": schedule}, f, indent=2, ensure_ascii=False)
    print("\nWrote ./branches/bus/meal/final_week_plan.json")

...
Overview:
  - Reads './branches/bus/energy/baseline_daily.json' + './branches/bus/energy/tou_split.json' + './branches/bus/energy/bill_breakdown.json' + './branches/bus/energy/savings_plan.json'.
  - Emits './branches/bus/energy/energy_report.json' and './branches/bus/energy/energy_report.csv'.
...
def main():
    ...
    ap.add_argument("--in_base", default="./branches/bus/energy/baseline_daily.json")
    ap.add_argument("--in_split", default="./branches/bus/energy/tou_split.json")
    ap.add_argument("--in_bill", default="./branches/bus/energy/bill_breakdown.json")
    ap.add_argument("--in_plan", default="./branches/bus/energy/savings_plan.json")
    args = ap.parse_args()
    ...
    json.dump(report, open("./branches/bus/energy/energy_report.json","w",encoding="utf-8"), indent=2)
    write_csv(report, "./branches/bus/energy/energy_report.csv")
    print("\nWrote ./branches/bus/energy/energy_report.json and ./branches/bus/energy/energy_report.csv")

#!/usr/bin/env -S ts-node
/*  Goal-driven Parallel Sequences – TypeScript port
    Original idea: Jos De Roo (Prolog)
    Python → JS translation by ChatGPT
    JS → TS adaptation: this file
*/

/* ──────────────────────────────────────────────────────────
   Basic types & helpers (Prolog-style unification)
   ────────────────────────────────────────────────────────── */

type Atom = string | number;        // either a symbol or a numeric constant
type Fact = Atom[];                 // e.g. ['location','S','gent']
type Env = Record<string, Atom>;    // variable ↦ concrete value

function isVar(token: Atom): token is string {
  return (
    typeof token === 'string' &&
    (token.startsWith('_') || /[A-Z]/.test(token[0]))
  );
}

/** Try to unify `pattern` with `fact` under (an optional) existing env. */
function unify(
  pattern: Fact,
  fact: Fact,
  env: Env = {}
): Env | null {
  if (pattern.length !== fact.length) return null;
  const e: Env = { ...env };

  for (let i = 0; i < pattern.length; i++) {
    const p = pattern[i];
    const f = fact[i];

    if (isVar(p)) {
      if (p in e && e[p] !== f) return null;
      e[p] = f;
    } else if (p !== f) {
      return null;
    }
  }
  return e;
}

function instantiate(pattern: Fact, env: Env): Fact {
  return pattern.map(tok =>
    typeof tok === 'string' && tok in env ? env[tok] : tok
  );
}

function factEquals(a: Fact, b: Fact): boolean {
  return a.length === b.length && a.every((v, i) => v === b[i]);
}

/* ──────────────────────────────────────────────────────────
   Domain object
   ────────────────────────────────────────────────────────── */

class Transition {
  constructor(
    public readonly mapId: string,
    public readonly fromPattern: Fact,
    public readonly toPattern: Fact,
    /** descriptive action id */
    public readonly action: string,
    public readonly duration: number,
    public readonly cost: number,
    public readonly belief: number,
    public readonly comfort: number
  ) {}
}

/* ──────────────────────────────────────────────────────────
   Utility predicates
   ────────────────────────────────────────────────────────── */

function stageCount(history: string[]): number {
  if (history.length === 0) return 1;
  let stages = 1;
  for (let i = 1; i < history.length; i++) {
    if (history[i] !== history[i - 1]) stages += 1;
  }
  return stages;
}

function goalSatisfied(state: Fact[], goalPattern: Fact): boolean {
  return state.some(fact => unify(goalPattern, fact) !== null);
}

function applyTransition(state: Fact[], tr: Transition, env: Env): Fact[] {
  const fromFact = instantiate(tr.fromPattern, env);
  const toFact   = instantiate(tr.toPattern,   env);

  const next = state.filter(f => !factEquals(f, fromFact)); // remove one
  next.push(toFact);
  return next;
}

function stateSignature(state: Fact[]): string {
  // stable string key for loop-checking
  return state
    .map(f => JSON.stringify(f))
    .sort()
    .join('|');
}

/* ──────────────────────────────────────────────────────────
   Depth-first search generator
   ────────────────────────────────────────────────────────── */

type Limits = [
  maxDuration: number,
  maxCost: number,
  minBelief: number,
  minComfort: number,
  maxStages: number
];

type YieldTuple = [
  path: string[],
  duration: number,
  cost: number,
  belief: number,
  comfort: number
];

function* dfs(
  state: Fact[],
  transitions: Transition[],
  goalPattern: Fact,
  limits: Limits,
  mapHist: string[],
  pathHist: string[],
  dur: number,
  cost: number,
  belief: number,
  comfort: number,
  visited: Set<string>
): Generator<YieldTuple> {
  const sig = stateSignature(state);
  if (visited.has(sig)) return;
  visited.add(sig);

  if (goalSatisfied(state, goalPattern)) {
    yield [pathHist, dur, cost, belief, comfort];
  }

  const [maxDur, maxCost, minBelief, minComfort, maxStages] = limits;

  for (const tr of transitions) {
    if (stageCount([...mapHist, tr.mapId]) > maxStages) continue;

    for (const fact of state) {
      const env = unify(tr.fromPattern, fact);
      if (!env) continue;

      const dur2     = dur     + tr.duration;
      const cost2    = cost    + tr.cost;
      const belief2  = belief  * tr.belief;
      const comfort2 = comfort * tr.comfort;

      if (dur2     > maxDur)     continue;
      if (cost2    > maxCost)    continue;
      if (belief2  < minBelief)  continue;
      if (comfort2 < minComfort) continue;

      const nextState = applyTransition(state, tr, env);

      yield* dfs(
        nextState,
        transitions,
        goalPattern,
        limits,
        [...mapHist, tr.mapId],
        [...pathHist, tr.action],
        dur2,
        cost2,
        belief2,
        comfort2,
        new Set(visited)          // copy for back-tracking
      );
    }
  }
}

function* allPaths(
  state: Fact[],
  transitions: Transition[],
  goalPattern: Fact,
  limits: Limits
): Generator<YieldTuple> {
  yield* dfs(state, transitions, goalPattern, limits, [], [], 0, 0, 1, 1, new Set());
}

/* ──────────────────────────────────────────────────────────
   Example knowledge base (partial Belgium map)
   ────────────────────────────────────────────────────────── */

const transitions: Transition[] = [
  new Transition(
    'map_be',
    ['location', 'S', 'gent'],
    ['location', 'S', 'brugge'],
    'drive_gent_brugge',
    1500.0, 0.006, 0.96, 0.99
  ),
  new Transition(
    'map_be',
    ['location', 'S', 'gent'],
    ['location', 'S', 'kortrijk'],
    'drive_gent_kortrijk',
    1600.0, 0.007, 0.96, 0.99
  ),
  new Transition(
    'map_be',
    ['location', 'S', 'kortrijk'],
    ['location', 'S', 'brugge'],
    'drive_kortrijk_brugge',
    1600.0, 0.007, 0.96, 0.99
  ),
  new Transition(
    'map_be',
    ['location', 'S', 'brugge'],
    ['location', 'S', 'oostende'],
    'drive_brugge_oostende',
    900.0, 0.004, 0.98, 1.0
  )
];

const initialState: Fact[]  = [['location', 'i1', 'gent']];
const goalPattern:  Fact    = ['location', '_', 'oostende'];
const limits: Limits        = [5000.0, 5.0, 0.2, 0.4, 1];

/* ──────────────────────────────────────────────────────────
   Demo driver (mirrors the original Prolog query)
   ────────────────────────────────────────────────────────── */

if (require.main === module) {
  let idx = 1;
  for (const [path, dur, cost, bel, comf] of allPaths(
    initialState,
    transitions,
    goalPattern,
    limits
  )) {
    console.log(`Solution ${idx++}`);
    console.log('  Path     :', path.join(' → '));
    console.log('  Duration :', dur);
    console.log('  Cost     :', cost);
    console.log('  Belief   :', bel);
    console.log('  Comfort  :', comf);
    console.log();
  }
}

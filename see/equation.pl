...
'http://eyereasoner.github.io/see#equation'(A,B,C) :-
    findall(E,
...
% query
query('http://eyereasoner.github.io/see#equation'(x^2-3*x+2=0,x,_ANSWER)).
query('http://eyereasoner.github.io/see#equation'(cos(x)*(1-2*sin(x))=0,x,_ANSWER)).
query('http://eyereasoner.github.io/see#equation'(2^(2*x)-5*2^(x+1)+16=0,x,_ANSWER)).

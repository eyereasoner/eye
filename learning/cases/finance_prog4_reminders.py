#!/usr/bin/env python3
"""
EYE Learning — Program 4: Reminders & Calendar Export (Text/ICS-lite)
...
Overview:
  - Reads './resources/forecast_30d.json' and './resources/goals_plan.json'.
  - Emits a small reminders list and a minimal .ics-like text file.
  - Prints Answer/Reason/Check and writes './resources/reminders.json' and './resources/reminders.ics.txt'.
"""
...
def main():
    ...
    parser.add_argument("--in_forecast", default="./resources/forecast_30d.json")
    parser.add_argument("--in_goals", default="./resources/goals_plan.json")
    ...
    with open("./resources/reminders.json","w",encoding="utf-8") as f:
        json.dump(rem, f, indent=2, ensure_ascii=False)
    ics = write_ics_like(rem["items"])
    with open("./resources/reminders.ics.txt","w",encoding="utf-8") as f:
        f.write(ics)
    print("\nWrote ./resources/reminders.json and ./resources/reminders.ics.txt")

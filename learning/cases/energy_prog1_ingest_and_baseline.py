#!/usr/bin/env python3
"""
EYE Learning — Program 1: Ingest & Baseline (kWh/day)
...
Overview (matches this chat):
  - Reads './resources/house_usage_sample.json' (or your file via --in).
  - Normalizes per-appliance usage to kWh/day.
  - Computes daily cost and daily CO₂.
  - Prints Answer/Reasons/Check; writes './resources/baseline_daily.json' for Program 2.
...
def main():
    ...
    ap.add_argument("--in", dest="infile", default="./resources/house_usage_sample.json")
    ap.add_argument("--out", default="./resources/baseline_daily.json")
    args = ap.parse_args()

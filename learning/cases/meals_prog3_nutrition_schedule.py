...
#!/usr/bin/env python3
"""
EYE Learning — Program 3: Nutrition Summary & Simple Cooking Schedule
...
Overview:
  Consume meal_./resources/plan.json and ./resources/grocery_list.json; compute per-day calories/macros from recipe
  nutrition and confirm ingredients are covered by pantry + purchases. Also output a simple
  cooking order. Prints Answer/Reason/Check and writes ./resources/final_week_plan.json.

Run:
  python meals_prog3_nutrition_schedule.py --plan ./resources/meal_plan.json --groceries ./resources/grocery_list.json
"""
...
def main():
    ...
    ap.add_argument("--plan", default="./resources/meal_plan.json")
    ap.add_argument("--groceries", default="./resources/grocery_list.json")
    args = ap.parse_args()
    ...
    with open("./resources/final_week_plan.json","w",encoding="utf-8") as f:
        json.dump({"summary": summary, "schedule": schedule}, f, indent=2, ensure_ascii=False)
    print("\nWrote ./resources/final_week_plan.json")

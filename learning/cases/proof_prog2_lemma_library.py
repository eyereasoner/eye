#!/usr/bin/env python3
"""
EYE Learning — Program 2: Lemma Library (Facts the proofs will use)

Short story (header):
  Collect the tiny facts needed for each claim type (definitions + standard lemmas).

Overview:
  - Reads './resources/normalized_claims.json'.
  - Emits a minimal library of facts keyed by claim type.
  - Prints Answer/Reason/Check; writes './resources/lemma_library.json' for Program 3.
...
def main():
    ...
    ap.add_argument("--infile", default="./resources/normalized_claims.json")
    ap.add_argument("--out", default="./resources/lemma_library.json")
    args = ap.parse_args()

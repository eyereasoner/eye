...
def unify_term(a: Term, b: Term, σ: Subst):
    ...
    if a == b:          return σ
    if is_var(a):       return _extend(a, b, σ)
    if is_var(b):       return _extend(b, a, σ)
...
def _num(lit: Literal):
    if not isinstance(lit, Literal): return None
...
class Reasoner:
    ...
    def _extract_rules(self,g):
        ...
        for s,_,o in list(g.triples((None,LOG.implies,None))):
            _add(_graph_to_pats(g.value(o,LOG.graph),g),
                 _graph_to_pats(g.value(s,LOG.graph),g),"forward")
            _scrub(s,g); _scrub(o,g)
        for s,_,o in list(g.triples((None,LOG.isImpliedBy,None))):
            _add(_graph_to_pats(g.value(s,LOG.graph),g),
                 _graph_to_pats(g.value(o,LOG.graph),g),"backward")
            _scrub(s,g); _scrub(o,g)

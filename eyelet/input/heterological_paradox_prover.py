...
────────────────────────────────
Deterministic first‑order resolution proof of the Grelling–Nelson
paradox (“heterological adjectives”).

Predicate  A(x,y)  ≝  “adjective x applies to adjective y”
Constant   Heterological  ≝  the adjective “heterological”

Paradoxical axiom (adapted from Grelling–Nelson):
    ∀x [  A(Heterological,x)  ↔  ¬A(x,x)  ]

Clauses (variables implicitly ∀‑quantified):

1.  A(x,x)  ∨  A(Heterological,x)          (¬A(x,x) → A(H,x))
2.  ¬A(Heterological,x)  ∨  ¬A(x,x)        (A(H,x)  → ¬A(x,x))

Goal: **A(Heterological,Heterological)**  
We refute its negation ¬A(Heterological,Heterological).

Expected proof (always identical):

01. A(Heterological, Heterological)   (from ¬A(Heterological, Heterological) , A(Heterological, x) | A(x, x))
02. ⊥                                 (from A(Heterological, Heterological) , ¬A(Heterological, Heterological))
...
def prove(kb, neg_goal):
    sos = [neg_goal]                 # FIFO set‑of‑support – deterministic
...
KB_TEXT = [
    "A(x,x) | A(Heterological,x)",
    "¬A(Heterological,x) | ¬A(x,x)",
]
KB = [parse_clause(line) for line in KB_TEXT]

NEGATED_GOAL = parse_clause("¬A(Heterological,Heterological)")

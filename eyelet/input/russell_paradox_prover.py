...
─────────────────────────
A deterministic first‑order resolution proof that naïve set
comprehension yields the Russell paradox (⊥ in two steps).

Predicate  M(x,y)  ≝  “x is a member of y”

Russell set R :  (∀x) [ M(x,R) ↔ ¬M(x,x) ]

Clausal form (variables are implicitly ∀‑quantified):

1. M(x,x) ∨ M(x,Russell)                 (¬M(x,x) → M(x,R))
2. ¬M(x,Russell) ∨ ¬M(x,x)              (M(x,R)  → ¬M(x,x))

Goal to prove:  **M(Russell,Russell)**, so we refute its negation.
"""

import re, itertools

# ─────────────────────  Compact term / literal classes  ──────────────────────
...
Clause = frozenset                                           # set of Literals

# ────────────────────────  Super‑simple parser  ─────────────────────────────
def parse_literal(text: str) -> Literal:
    """Parse strings like  '¬M(x,Russell)'  or  'M(x,x)'."""
    text = text.strip()
    neg = text.startswith(('¬', '~'))
    if neg:
        text = text[1:].strip()
    ...
    args = [Term(a.strip()) for a in arg_str.split(',') if a.strip()]  # no function terms needed here
    return Literal(pred, args, neg)


def parse_clause(line: str) -> Clause:
    """Split on ‘|’, parse each literal, return a frozenset."""
    ...
    for Li in sorted(ci, key=repr):
        for Lj in sorted(cj, key=repr):
            if Li.pred == Lj.pred and Li.neg != Lj.neg:
                ...
                if any(L.negate() in resolvent for L in resolvent):     # tautology check
                    continue
                yield resolvent


def prove(kb, neg_goal):
    sos = [neg_goal]                        # FIFO – deterministic
...
NEGATED_GOAL = parse_clause("¬M(Russell,Russell)")
